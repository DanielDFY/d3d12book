...
// Exercise_17_01 PickingApp.cpp modified by DanielDFY
...
void PickingApp::Pick(int sx, int sy) {
	...
	for (auto ri : mRitemLayer[(int)RenderLayer::Opaque]) {
		...
		// If we hit the bounding sphere of the Mesh, then we might have picked a Mesh triangle,
		// so do the ray/triangle tests.
		//
		// If we did not hit the bounding sphere, then it is impossible that we hit 
