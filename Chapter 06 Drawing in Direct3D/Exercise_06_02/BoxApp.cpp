...
// Modify: Now vertex position and color data are separated
...
void BoxApp::BuildBoxGeometry() {
    ...
    std::array<VertexPosData, 8> vertexPositions =
    {
    ...
    std::array<VertexColorData, 8> vertexColors =
    {
    ...
    const UINT vbPosByteSize = static_cast<UINT>(vertexPositions.size() * sizeof(VertexPosData));
    const UINT vbColorByteSize = static_cast<UINT>(vertexColors.size() * sizeof(VertexColorData));
    ...
    CopyMemory(mBoxGeo->VertexPosBufferCPU->GetBufferPointer(), vertexPositions.data(), vbPosByteSize);

    ThrowIfFailed(D3DCreateBlob(vbColorByteSize, &mBoxGeo->VertexColorBufferCPU));
    CopyMemory(mBoxGeo->VertexColorBufferCPU->GetBufferPointer(), vertexColors.data(), vbColorByteSize);
    ...
    mBoxGeo->VertexPosBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(),
        mCommandList.Get(), vertexPositions.data(), vbPosByteSize, mBoxGeo->VertexPosBufferUploader);

    mBoxGeo->VertexColorBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(),
        mCommandList.Get(), vertexColors.data(), vbColorByteSize, mBoxGeo->VertexColorBufferUploader);
    /*
