...
// Exercise_19_05 NormalMapApp.cpp modified by DanielDFY
...
enum class RenderLayer : int
{
	Opaque = 0,
	Sky,
	Wave,	// Modify: add render layer for wave
	Count
};
...
void NormalMapApp::Draw(const GameTimer& gt)
{
	...
	// Modify: add PSO and render items for wave rendering
	mCommandList->SetPipelineState(mPSOs["wave"].Get());
	DrawRenderItems(mCommandList.Get(), mRitemLayer[static_cast<int>(RenderLayer::Wave)]);
...
{
	auto waterMat = mMaterials["wave"].get();
...
void NormalMapApp::LoadTextures()
{
	std::vector<std::string> texNames = 
	{
		...
		// Modify: add albedo, displacement and normal textures for wave
		"waterDiffuseMap",
		"waveNormalMap0",
		"waveHeightMap0",
		"waveNormalMap1",
	...
	std::vector<std::wstring> texFilenames = 
	{
		...
		L"../../Textures/wave0_nmap.dds",
		L"../../Textures/wave0_dmap.dds",
		L"../../Textures/wave1_nmap.dds",
		L"../../Textures/wave1_dmap.dds",
...
void NormalMapApp::BuildDescriptorHeaps()
{
	...
	std::vector<ComPtr<ID3D12Resource>> tex2DList = 
	{
		...
		mTextures["waveNormalMap0"]->Resource,
		mTextures["waveHeightMap0"]->Resource,
	...
	for(UINT i = 0; i < static_cast<UINT>(tex2DList.size()); ++i)
	{
...
void NormalMapApp::BuildShapeGeometry()
{
    ...
    const UINT vbByteSize = static_cast<UINT>(vertices.size()) * static_cast<UINT>(sizeof(Vertex));
    const UINT ibByteSize = static_cast<UINT>(indices.size()) * static_cast<UINT>(sizeof(std::uint16_t));
...
void NormalMapApp::BuildPSOs()
{
	...
	wavePsoDesc.BlendState.RenderTarget[0] = transparencyDesc;
...
void NormalMapApp::BuildMaterials()
{
	...
	// Modify: add material for wave
	auto wave = std::make_unique<Material>();
	wave->Name = "wave";
	wave->MatCBIndex = 1;
	wave->DiffuseSrvHeapIndex = 2;
	wave->NormalSrvHeapIndex = 3;
	wave->DiffuseAlbedo = XMFLOAT4(0.0f, 0.2f, 0.6f, 0.7f);
	wave->FresnelR0 = XMFLOAT3(0.1f, 0.1f, 0.1f);
	wave->Roughness = 0.1f;
...
void NormalMapApp::BuildRenderItems()
{
	...
	mRitemLayer[static_cast<int>(RenderLayer::Opaque)].push_back(globeRitem.get());
	...
	gridRitem->Mat = mMaterials["wave"].get();
	...
	mRitemLayer[static_cast<int>(RenderLayer::Wave)].push_back(gridRitem.get());
