...
// Exercise_08_06 Default.hlsl modified by DanielDFY
...
float4 PS(VertexOut pin) : SV_Target
{
    ...
    // Modify: now apply cartoon styled lighting
    // kd
    float4 diffuseAlbedo = gMaterialConstants.gDiffuseAlbedo;
    for (int i = 0; i < 3; ++i) {
        if (diffuseAlbedo[i] <= 0) {
            diffuseAlbedo[i] = 0.4f;
        } else if (diffuseAlbedo[i] <= 0.5f) {
            diffuseAlbedo[i] = 0.6f;
        } else {
            diffuseAlbedo[i] = 1.0f;
        }
    }
